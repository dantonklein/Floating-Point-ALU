...
class fp_item;
    rand bit [31:0] in1_bits, in2_bits, in3_bits;
    rand bit [2:0] rounding_mode;
    rand bit special_case1, special_case2, special_case3;
    ...
    function void post_randomize();
        if(special_case1) in1_bits = get_random_special();
        if(special_case2) in2_bits = get_random_special();
        if(special_case3) in3_bits = get_random_special();
    endfunction
endclass

module fused_mult_add_tb #(
    ...
    initial begin
        ...
        for(int i = 0; i < NUM_TESTS; i++) begin
            ...
            rounding_mode <= RNE;
            valid_data_in <= 1;
            expected_out = $bitstoshortreal(item.in1_bits) * $bitstoshortreal(item.in2_bits) + $bitstoshortreal(item.in3_bits);
            @(posedge clk);
            $display("[%0t] Test %0d: in1=0x%08h in1float=%f in2=0x%08h in2float=%f in3=0x%08h in3float=%f rmode=%0d", $time, i, in1, $bitstoshortreal(in1), in2, $bitstoshortreal(in2), in3, $bitstoshortreal(in3), rounding_mode);
            valid_data_in <= 0;
